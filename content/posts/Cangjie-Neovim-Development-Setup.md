...
### 创建 GCC 运行时库符号链接 {#创建-GCC-运行时库符号链接}
...
### 1. 安装 AstroNvim {#1-安装-AstroNvim}

```bash
git clone --depth 1 https://github.com/AstroNvim/AstroNvim ~/.config/nvim
```

### 2. 配置仓颉 LSP 插件 {#2-配置仓颉-LSP-插件}
...
### 3. 启动 Neovim 并安装插件 {#3-启动-Neovim-并安装插件}
...
### 1. 链接错误：cannot find crtbeginS.o {#1-链接错误：cannot-find-crtbeginS-o}
