...
- matebook pro
categories: 软件
slug: matebookpro-run-vite7-vue-rollup
---
...
### 1. 修改Rollup的native.js文件 {#1-修改Rollup的native-js文件}
...
### 2. package.json配置 {#2-package-json配置}
确保使用Vite 7及相关插件：
```json
{
  "devDependencies": {
...
### 方案A：降级Vite到6.x {#方案A：降级Vite到6-x}
- 优点：完全兼容Node.js 22.7.0
- 缺点：无法使用Vite 7新特性
- 操作：修改package.json中Vite版本为`^6.0.0`， 然后一样修改Rollup的native.js文件

### 方案B：升级Node.js {#方案B：升级Node-js}
- 优点：完全符合Vite 7要求
- 缺点：鸿蒙系统无法升级Node.js
- 操作：将Node.js升级到22.12+

### 方案C：使用环境变量 {#方案C：使用环境变量}
尝试过但未成功：
```bash
