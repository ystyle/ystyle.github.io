...
lastmod: 2026-09-09 23:30:00
slug: chronomem-oom-hunt
categories:
  - 软件
tags:
  - Cangjie
...
## 第一现场：写路径为什么会静悄悄地断 {#第一现场：写路径为什么会静悄悄地断}
...
## 第二现场：256MB 的 GC 堆与 4MB 的 memtable {#第二现场：256MB-的-GC-堆与-4MB-的-memtable}
...
## 顺着依赖往上挖：badger-cj、json-rpc、mcp-cj {#顺着依赖往上挖：badger-cj、json-rpc、mcp-cj}

线上服务本体只是一个薄薄的 MCP 服务层，真正的故事发生在它脚下的**自研库依赖链**里。这一轮排查像剥洋葱，每剥一层都发现一个独立的缺陷。

### badger-cj：存储引擎的三笔旧账 {#badger-cj：存储引擎的三笔旧账}
...
### json-rpc：被 OOM 杀死的接收循环 {#json-rpc：被-OOM-杀死的接收循环}
...
### SSE：`wait()` 永远等不到的那个通知 {#SSE：wait-永远等不到的那个通知}
...
### cjxt：同一款病，换了个协议 {#cjxt：同一款病，换了个协议}
...
### cron-cj：差点被漏掉的第三个循环 {#cron-cj：差点被漏掉的第三个循环}
